...
let githubRepo: URL;
try {
	logger.log("Parsing the repository URL");

	githubRepo = new URL(packageJSON.repository.url);
} catch (err) {
	throw new Error(err instanceof TypeError ? `Failed to parse the repository URL: ${err.message}` : `Unexpected error occured while parsing the repository URL: ${err.message}`);
}

logger.log("Getting the owner and repository name from the URL");

const [owner, repoRaw] = githubRepo.pathname.split("/").slice(1);
const repoSplit = repoRaw.split(".git");
if (repoSplit.at(-1) !== "")
	throw new Error("Invalid format for the GitHub repository URL, expected a .git at the end of the URL");
if (repoSplit.length !== 2)
	throw new Error("Unexpected double split of \".git\" in the owner field of the repository URL");
